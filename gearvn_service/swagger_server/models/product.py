...
class Product(Model):
        ...
        for product in products:
            ...
            for q in query_splitted:
                if q.lower() not in name_low:
                    ...
                    break
            if ok:
                url = f"https://gearvn.com/products/{product['handle']}"
                webpage = requests.get(url, headers=headers)
                soup = BeautifulSoup(webpage.content, "html.parser")
                image = soup.find(id="detail-product").find('img').get('src')
                res.append({
                    'name': product['title'],
                    'price': product['price'],
                    'image': f'https:{image}',
    ...
    def search_products(query):
        ...
        response = requests.post(
            'https://gearvn.com/apps/gvn_search/search_products', headers=headers, data=json.dumps(body))
